...
$(() => {
    ...
    socket.on("user_connection", (data) => {
        ...
        let userToken = localStorage.getItem('token') || prompt("What is your token?");
            ...
            switch (command) {
                ...
                case "//room":
                    ...
                    switch (args[0].toLowerCase()) {
                        case "change":
                            if (!args[1]) return append("Usage: //room change <room id> <room name>");
                            if (!args[2]) return append("Usage: //room change <room id> <room name>");
                            if (args[1].trim() == "") return append("Usage: //room change <room id> <room name>");
